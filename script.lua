...
getgenv().WindUI:Popup({
    ...
    Icon = "sparkles",
    IconThemed = true,
    Content = "Chào mừng bạn đến với " .. gradient("Khoa Dev - Avade", Color3.fromHex("#6A11CB"), Color3.fromHex("#2575FC")) .. ".",
    Buttons = {
        {
            Title = "Tiếp Tục",
            Icon = "arrow-right",
...
local Window = getgenv().WindUI:CreateWindow({
    ...
    OpenButton = {
        Title = "Mở Khoa Dev - Avade",
        CornerRadius = UDim.new(1,0),
        StrokeThickness = 3,
        Enabled = true,
        OnlyMobile = false,
        Color = ColorSequence.new(
            Color3.fromHex("#FF306A"), 
            Color3.fromHex("#ff2f2f")
        ),
    },
})

Window:CreateTopbarButton("theme-switcher", "moon", function()
    getgenv().WindUI:SetTheme(getgenv().WindUI:GetCurrentTheme() == "Dark" and "Light" or "Dark")
    getgenv().WindUI:Notify({
        Title = "Đã Đổi Giao Diện",
        Content = "Giao diện hiện tại: "..getgenv().WindUI:GetCurrentTheme(),
        Duration = 2
    })
end, 990)

local FeaturesSection = Window:Section({ Title = "Tính Năng Chính", Opened = true })
local PlayerTab = Window:Tab({Title = "Player", Icon = "user"})
local AutoTab = Window:Tab({Title = "Auto", Icon = "play"})
local FPSTab = Window:Tab({Title = "FPS", Icon = "zap"})

local VisualsSection = Window:Section({ Title = "Hiển Thị & ESP", Opened = true })
local EspTab = Window:Tab({Title = "Esp", Icon = "eye"})
local VisualTab = Window:Tab({Title = "Hình Ảnh", Icon = "palette"})

local SettingsSection = Window:Section({ Title = "Cài Đặt", Opened = true })
local InfoTab = Window:Tab({Title = "Thông Tin", Icon = "info"})
local SettingsTab = Window:Tab({Title = "Thiết Lập", Icon = "settings"})

-- ... (bagian awal kode tetap sama) ...

getgenv().featureToggles = {}

local function createGradientButton(parent, name, initialState, yPosition, toggleCallback)
    ...
    button.Position = UDim2.new(0.02, 0, yPosition, 0)
    ...
    if initialState then
        gradient.Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0, Color3.fromRGB(50, 200, 100)),  -- Hijau cerah
            ColorSequenceKeypoint.new(1, Color3.fromRGB(20, 120, 60))   -- Hijau gelap
        }
    else
        gradient.Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0, Color3.fromRGB(200, 200, 200)),  -- Putih terang
            ColorSequenceKeypoint.new(1, Color3.fromRGB(150, 150, 150))   -- Abu-abu
        }
    end
    ...
    if initialState then
        stroke.Color = Color3.fromRGB(0, 180, 80)  -- Hijau untuk ON
    else
        stroke.Color = Color3.fromRGB(100, 100, 100)  -- Abu-abu untuk OFF
    end
    stroke.Thickness = 2
    ...
    if initialState then
        label.TextColor3 = Color3.fromRGB(255, 255, 255)  -- Putih untuk ON
    else
        label.TextColor3 = Color3.fromRGB(240, 240, 240)  -- Putih cerah untuk OFF
    end
    label.TextSize = 14
    ...
    clicker.MouseEnter:Connect(function()
        if initialState then
            stroke.Color = Color3.fromRGB(0, 220, 100)  -- Hijau lebih cerah saat hover
        else
            stroke.Color = Color3.fromRGB(130, 130, 130)  -- Abu-abu lebih terang saat hover
        end
    end)
...
InfoTab:Section({ Title = "Tham Gia Máy Chủ Discord", TextSize = 20 })
InfoTab:Divider()

InfoTab:Paragraph({
    Title = "Khoa Dev - Avade Discord",
    Desc = "Tham gia máy chủ Discord của chúng tôi để nhận các bản cập nhật mới, hỗ trợ và giao lưu với cộng đồng!",
    ...
    Buttons = {
        {
            ...
            Callback = function()
                ...
                getgenv().WindUI:Notify({
                    Title = "Discord",
                    Content = "Đã sao chép liên kết Discord vào bộ nhớ tạm!",
                    Duration = 3
                })
...
InfoTab:Section({ Title = "Giới Thiệu", TextSize = 20 })
InfoTab:Divider()

InfoTab:Paragraph({
    Title = "Tác Giả & Đóng Góp",
    Desc = "Kịch bản: Khoa Dev - Avade\nNgười viết kịch bản gốc: SYFTA\nThư viện UI: WindUI\nCảm ơn chân thành đến toàn bộ tester và người ủng hộ!",
    Image = "https://cdn.discordapp.com/attachments/1203333550010200175/1281053540044574741/download.png?ex=66fcb6c5&is=66fb6545&hm=12ec10e94d84ff8e06fd14e123796eb775ac48eb3daa2f6bd285626aabf8c0c0&",
    ImageSize = 48,
    Buttons = {}
})
...
PlayerTab:Toggle({
    Title = "Tự Động Nhảy Khi Lao Tới",
    Value = true,
    Callback = function(state)
...
PlayerTab:Input({
    Title = "Tùy Chỉnh Chiều Cao Nhảy (Jump Cap)",
    Icon = "chevrons-up",
...
PlayerTab:Input({
    Title = "Gia Tốc Khi Không Trung (Strafe)",
    Icon = "wind",
    Placeholder = "Mặc định 187",
    Value = "187",
    Callback = function(input)
        local val = tonumber(input)
...
PlayerTab:Toggle({
    Title = "Trượt Vô Hạn (Infinite Slide)",
    Value = false,
    Callback = function(state)
...
PlayerTab:Input({
    Title = "Độ Ma Sát Khi Trượt",
    Desc = "Chỉ số âm để trượt nhanh hơn",
    Placeholder = "-8",
    NumbersOnly = true,
    Value = "-8",
...
PlayerTab:Toggle({
    Title = "Tự Động Nhảy Liên Tục (Bhop)",
    Value = false,
    Callback = function(state)
...
PlayerTab:Section({ Title = "Tốc Độ Emote (BETA)", TextSize = 20 })
PlayerTab:Divider()
...
PlayerTab:Dropdown({
    Title = "Chế Độ Tốc Độ Emote",
    Values = {"Tắt", "Hợp Lệ (Legit)", "Nhân Tốc Độ (Multiplier)"},
    Value = getgenv().EmoteSpeedSettings.Mode == "not this" and "Tắt" or (getgenv().EmoteSpeedSettings.Mode == "Legit" and "Hợp Lệ (Legit)" or "Nhân Tốc Độ (Multiplier)"),
    Callback = function(value)
        local mode = (value == "Tắt" and "not this") or (value == "Hợp Lệ (Legit)" and "Legit") or "Multiplier speed"
        getgenv().EmoteSpeedSettings.Mode = mode
        
        if mode == "not this" then
...
PlayerTab:Button({
    Title = "Áp Dụng Cho Emote Đứng Yên (Unwalkable)",
    Callback = function()
        if not itemsFolder then return end
        ...
        getgenv().WindUI:Notify({
            Title = "Tốc Độ Emote",
            Content = "Đã áp dụng tốc độ cho các Emote đứng yên!",
            Duration = 3
        })
    end
})
...
PlayerTab:Button({
    Title = "Đặt Lại Tốc Độ Emote",
    Desc = "Gặp lỗi? Hãy đặt lại tốc độ hoặc vào lại game",
    Callback = function()
        restoreOriginalEmoteSpeeds()
        resetMultiplierSpeed()
        ...
        getgenv().WindUI:Notify({
            Title = "Tốc Độ Emote",
            Content = "Đã đặt lại tất cả tốc độ Emote về mặc định!",
            Duration = 3
        })
    end
})
...
PlayerTab:Button({
    Title = "Lưu Cài Đặt Hiện Tại",
    Callback = function()
        getgenv().WindUI:Notify({
            Title = "Tốc Độ Emote",
            Content = "Đã lưu cài đặt! Tốc độ: " .. getgenv().EmoteSpeedSettings.SpeedValue .. 
                     ", Chế độ: " .. getgenv().EmoteSpeedSettings.Mode,
            Duration = 3
        })
    end
})
...
AutoTab:Toggle({
    Title = "Tự Động Vác Đồng Đội (Auto Carry)",
    Value = false,
    Callback = function(state)
...
AutoTab:Toggle({
    Title = "Cứu Nhanh Khi Đang Emote",
    Value = false,
    Callback = function(state)
...
AutoTab:Slider({
    Title = "Độ Trễ Cứu",
    Desc = "Khoảng thời gian chờ giữa mỗi lần cứu",
...
AutoTab:Toggle({
    Title = "Tự Động Hồi Sinh Bản Thân",
    Value = false,
    Callback = function(state)
...
AutoTab:Dropdown({
    Title = "Phương Thức Tự Hồi Sinh",
    Values = {"Điểm Hồi Sinh (Spawnpoint)", "Hồi Sinh Tại Chỗ (Fake Revive)"},
    Value = getgenv().FeatureStates.SelfReviveMethod == "Spawnpoint" and "Điểm Hồi Sinh (Spawnpoint)" or "Hồi Sinh Tại Chỗ (Fake Revive)",
    Callback = function(value)
        local method = (value == "Điểm Hồi Sinh (Spawnpoint)" and "Spawnpoint" or "Fake Revive")
        getgenv().FeatureStates.SelfReviveMethod = method
    end
})
...
AutoTab:Button({
    Title = "Tự Hồi Sinh Thủ Công",
    Desc = "Nhấn để tự hồi sinh chính mình ngay lập tức",
    Icon = "heart",
...
AutoTab:Toggle({
    Title = "Chống Treo Máy (Anti AFK)",
    Desc = "Ngăn việc bị kích khỏi phòng do không hoạt động",
...
AutoTab:Toggle({
    Title = "Tự Động Thu Thập Vé (Ticket Farm)",
    Desc = "Tự động dịch chuyển nhặt các vé xuất hiện trên bản đồ",
    Value = false,
    Callback = function(state)
...
local function startAutoMoneyFarm()
    ...
    if not securityPart then
        getgenv().WindUI:Notify({
            Title = "Tự Động Cày Tiền",
            Content = "Không tìm thấy SecurityPart bảo an!",
            Duration = 3
        })
        return
    end
...
AutoTab:Toggle({
    Title = "Tự Động Cày Tiền (Auto Money)",
    Desc = "Tự động cứu người chơi khác để nhận tiền thưởng",
    Value = false,
    Callback = function(state)
...
FPSTab:Button({  
    Title = "Giảm Tải Vùng Bản Đồ (Lower Chunks)",  
    Description = "Giảm bán kính hiển thị môi trường xung quanh để mượt hơn",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Mở Khóa Giới Hạn FPS (Disable VSync)",  
    Description = "Tắt đồng bộ dọc của game để đạt FPS tối đa",  
    Callback = function()  
        setfpscap(9999)  
    end  
})  
...
FPSTab:Button({  
    Title = "Đặt Giới Hạn FPS Cực Hạn",  
    Description = "Thiết lập giới hạn FPS lên mức tối đa (99999)",  
    Callback = function()  
        setfpscap(99999)  
    end  
})  
...
FPSTab:Button({  
    Title = "Ẩn Bầu Trời (Hide Skybox)",  
    Description = "Ẩn kết cấu bầu trời để tối ưu dung lượng hiển thị",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Tắt Đổ Bóng (Disable Shadows)",  
    Description = "Tắt toàn bộ bóng đổ trong thế giới game",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Bật Đồ Họa Cực Thấp (Low Graphics)",  
    Description = "Đặt chất lượng kết xuất đồ họa về mức thấp nhất",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Tắt Hiệu Ứng Hạt (Disable Particles)",  
    Description = "Loại bỏ hoàn toàn các hiệu ứng hạt xung quanh",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Chuyển Thời Gian: Ban Đêm",  
    Description = "Chuyển thời gian hiển thị thành 00:00 đêm",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Chuyển Thời Gian: Ban Ngày",  
    Description = "Chuyển thời gian hiển thị thành 08:00 sáng",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Xóa Sương Mù (Anti Fog)",  
    Description = "Loại bỏ sương mù trong game giúp nhìn xa hơn",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Chế Độ Đơn Sắc (Low Poly)",  
    Description = "Chuyển các khối địa hình thành nhựa trơn bóng để tăng FPS",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Tối Giản Chiếu Sáng",  
    Description = "Vô hiệu hóa cấu hình chiếu sáng nâng cao",  
    Callback = function()  
        local L = game.Lighting  
...
FPSTab:Button({  
    Title = "Ẩn Chunks Không Sử Dụng",  
    Description = "Giảm tải các vùng bản đồ không hoạt động",  
    Callback = function()  
...
FPSTab:Button({  
    Title = "Giảm Khử Răng Cưa",  
    Description = "Tối thiểu hóa tính năng khử răng cưa để cải thiện tốc độ",  
    Callback = function()  
...
FPSTab:Toggle({
    Title = "Bật Sáng Bản Đồ (Full Bright)",
    Desc = "Giúp bản đồ luôn sáng rõ kể cả ban đêm",
    Callback = function()
...
FPSTab:Button({
    Title = "Giảm Đồ Họa V1",
    Desc = "Làm trơn toàn bộ vật thể, tắt hiệu ứng hạt và vệt sáng",
    Icon = "image-off",
    Callback = function()
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("BasePart") then
...
FPSTab:Button({
    Title = "Giảm Đồ Họa V2 (Tối Ưu)",
    Desc = "Mượt hơn nữa + Loại bỏ sương mù và các hiệu ứng đi kèm",
    Callback = function()
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("BasePart") then
                obj.Material = Enum.Material.SmoothPlastic
        ...
        getgenv().WindUI:Notify({
            Title = "Đồ Họa Được Tối Ưu",
            Content = "Đã áp dụng tối ưu hóa V2 thành công!",
            Duration = 3
        })
    end
})
...
FPSTab:Button({
    Title = "Sửa Lag & Tối Ưu Hóa Tổng Thể",
    Desc = "Giảm đồ họa, tối ưu hóa đường truyền mạng và dọn sạch mảnh vỡ",
    Icon = "zap",
    Callback = function()
        optimizeGraphics()
        optimizeNetwork()
        ...
        getgenv().WindUI:Notify({
            Title = "✅ Đã Áp Dụng Sửa Lag",
            Content = "Hiệu năng game đã được tối ưu hóa thành công!",
            Duration = 3
        })
    end
})
...
FPSTab:Button({
    Title = "Tự Tối Ưu Vật Thể Mới",
    Desc = "Tự động vô hiệu hóa các hạt/vệt sáng mới xuất hiện",
    Default = false,
    Callback = function(state)
        if state then
            workspace.DescendantAdded:Connect(function(obj)
...
FPSTab:Toggle({
    Title = "Tăng Tốc FPS Tối Đa (Tắt Kết Xuất 3D)",
    Desc = "Tắt render 3D để treo máy hoặc lấy tối đa FPS (Màn hình sẽ đen)",
    Default = false,
    Callback = function(state)
        local RunService = game:GetService("RunService")
...
local function createTimerUI()
    ...
    clientLabel.Text = "Thời gian chơi: 00:00"
    ...
    getgenv().GameTimerUI.Connection = RunService.Heartbeat:Connect(function()
        ...
        if timeLabel and clientLabel then
            if statsFolder then
                ...
                timeLabel.Text = "Thời gian: " .. (timerValue and tostring(math.floor(timerValue)) or "0")
            else
                timeLabel.Text = "Thời gian: 0"
            end
            
            local clientTime = tick() - getgenv().GameTimerUI.StartTime
            clientLabel.Text = "Thời gian chơi: " .. formatTime(clientTime)
        end
    end)
end
...
VisualTab:Toggle({
    Title = "Hiện Thời Gian Trận Đấu",
    Desc = "Hiển thị thời gian trận đấu và thời gian chơi",
...
VisualTab:Slider({
    Title = "Góc Nhìn (FOV)",
    Desc = "Điều chỉnh góc nhìn của bạn",
...
VisualTab:Input({
    Title = "ID Nhạc Nền",
    Desc = "Nhập ID nhạc từ Roblox để phát",
    Value = "",
    PlaceholderText = "Ví dụ: 1837532419",
    ...
    Callback = function(musicId)
        ...
        if musicId == "" or not tonumber(musicId) then
            WindUI:Notify({
                Title = "Lỗi",
                Content = "Vui lòng nhập ID nhạc hợp lệ!",
                Icon = "alert-circle",
                Duration = 3
            })
            return
        ...
        WindUI:Notify({
            ...
            Content = "Đang phát nhạc với ID: " .. musicId,
            Icon = "music",
            Duration = 3
        })
    end
...
VisualTab:Button({
    Title = "Tắt Nhạc",
    Desc = "Dừng phát tất cả các bản nhạc",
    Callback = function()
        ...
        WindUI:Notify({
            Title = "Đã Tắt Nhạc",
            Content = "Đã dừng phát " .. stoppedCount .. " bản nhạc",
            Icon = "stop-circle",
            Duration = 3
        })
    end
...
VisualTab:Toggle({
    Title = "Mở Khóa Battlepass",
    Desc = "Mở khóa giao diện và phần thưởng thẻ thành tích khi mua sắm",
    Value = false,
    Callback = function(state)
        if state then
...
VisualTab:Input({
    Title = "Giãn Camera Dọc",
    Value = "0.80",
    Placeholder = "Nhập giá trị giãn dọc",
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
...
VisualTab:Input({
    Title = "Giãn Camera Ngang",
    Value = "0.80",
    Placeholder = "Nhập giá trị giãn ngang",
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
...
VisualTab:Toggle({
    Title = "Giãn Màn Hình",
    Desc = "Điều chỉnh tỷ lệ kéo giãn dọc của camera",
    Value = false,
    Callback = function(state)
...
VisualTab:Toggle({
    Title = "Người Không Đầu & Chân Korblox",
    Desc = "Kích hoạt đầu vô hình (Headless) và chân xương (Korblox)",
...
VisualTab:Toggle({
    Title = "Sửa Hoạt Ảnh Cola",
    Desc = "Khắc phục lỗi đơ hoạt ảnh khi uống nước ngọt (Cola)",
    ...
    Callback = function(state)
        ...
        if state then
            ...
            local success, errorMsg = pcall(function()
                ...
                -- Cek apakah hook đã được cài đặt
                if getgenv().FeatureStates.FixColaHookEnabled then
                    getgenv().WindUI:Notify({
                        Title = "Sửa Hoạt Ảnh",
                        Content = "Đã áp dụng sửa lỗi hoạt ảnh trước đó!",
                        Duration = 3
                    })
                    return
                end
            ...
            if success then
                getgenv().WindUI:Notify({
                    Title = "Sửa Hoạt Ảnh",
                    Content = "Đã sửa hoạt ảnh Cola thành công!",
                    Duration = 3
                })
            else
                getgenv().WindUI:Notify({
                    Title = "Lỗi Sửa Hoạt Ảnh",
                    Content = "Không thể sửa hoạt ảnh: " .. tostring(errorMsg),
                    Duration = 5
                })
            ...
            if getgenv().FeatureStates.FixColaHookEnabled and getgenv().ColaFixData then
                ...
                getgenv().WindUI:Notify({
                    Title = "Sửa Hoạt Ảnh",
                    Content = "Đã gỡ bỏ sửa hoạt ảnh Cola!",
                    Duration = 3
                })
            end
        end
...
VisualTab:Input({  
    Title = "Vật Phẩm Hiện Tại",  
    Placeholder = "Nhập tên vật phẩm đang dùng",  
    Callback = function(v)
        if v == "" then
            cosmetic1 = ""
            return
        ...
        if match then
            cosmetic1 = match
            getgenv().WindUI:Notify({Title = "Thay Đổi Vật Phẩm", Content = "Vật phẩm hiện tại: " .. match})
            else
            cosmetic1 = ""
            getgenv().WindUI:Notify({Title = "Thay Đổi Vật Phẩm", Content = "Không tìm thấy vật phẩm này!"})
        end
    end  
})  

VisualTab:Input({  
    Title = "Chọn Vật Phẩm Muốn Đổi",  
    Placeholder = "Ví dụ: Toxic Inf",  
    Callback = function(v)
        if v == "" then
            cosmetic2 = ""
            return
        ...
        if match then
            cosmetic2 = match
            getgenv().WindUI:Notify({Title = "Thay Đổi Vật Phẩm", Content = "Đã chọn: " .. match})
        else
            cosmetic2 = ""
            getgenv().WindUI:Notify({Title = "Thay Đổi Vật Phẩm", Content = "Không tìm thấy vật phẩm này!"})
        end
...
VisualTab:Button({  
    ...
    Callback = function()  
        pcall(function()  
            ...
            getgenv().WindUI:Notify({Title = "Thay Đổi Vật Phẩm", Content = "Thay đổi vật phẩm thành công!"})
        end)    
    end  
})
...
for i = 1,6 do
    VisualTab:Input({
        Title = "Hành Động Hiện Tại " .. i,
        Placeholder = "Nhập tên hành động hiện tại",
        ...
        Callback = function(v)
            ...
            if match then
                ...
                getgenv().WindUI:Notify({Title = "Thay Đổi Hành Động", Content = "Hành động slot " .. i .. " hiện tại: " .. match})
            else
                getgenv().currentEmotes[i] = ""
                getgenv().WindUI:Notify({Title = "Thay Đổi Hành Động", Content = "Không tìm thấy hành động này ở slot " .. i})
            end
        end
    })

    VisualTab:Input({
        Title = "Chọn Hành Động Muốn Đổi " .. i,
        Placeholder = "Nhập tên hành động muốn đổi",
        ...
        Callback = function(v)
            ...
            if match then
                ...
                getgenv().WindUI:Notify({Title = "Thay Đổi Hành Động", Content = "Đã chọn cho slot " .. i .. ": " .. match})
            else
                getgenv().selectEmotes[i] = ""
                getgenv().WindUI:Notify({Title = "Thay Đổi Hành Động", Content = "Không tìm thấy hành động này ở slot " .. i})
            end
    ...
    VisualTab:Button({
        ...
        Callback = function()
            ...
            getgenv().WindUI:Notify({Title = "Thay Đổi Hành Động", Content = "Đã áp dụng cho ô hành động " .. i .. "!"})
        end
    })
...
VisualTab:Button({
    Title = "Đặt Lại Tất Cả Hành Động",
    ...
    Callback = function()
        ...
        getgenv().WindUI:Notify({Title = "Thay Đổi Hành Động", Content = "Đã đặt lại toàn bộ ô hành động!"})
    end
})
...
VisualTab:Input({
    Title = "Streak Giả (Fake Streak)",
    Placeholder = "Nhập số chuỗi thắng (Streak)",
    Callback = function(value)
        local num = tonumber(value)
        if num then
            pcall(function()
                ...
                getgenv().WindUI:Notify({
                    Title = "Chuỗi Thắng (Streak)",
                    Content = string.format("Đã chỉnh chuỗi thắng thành %d", num),
                    Duration = 3
                })
            end)
        end
...
SettingsTab:Section({Title = "Thiết Lập Hệ Thống"})
...
SettingsTab:Input({
    Title = "Ảnh/Video Nền Giao Diện",
    Value = currentBackground:match("%d+") or "",
    Placeholder = "Asset ID hoặc Link Video",
    Callback = applyBackground
})

SettingsTab:Dropdown({
    Title = "Hình Nền Khuyên Dùng",
...
SettingsTab:Toggle({
    Title = "Bật/Tắt Độ Trong Suốt Cửa Sổ",
    Callback = function(e)
...
local myConfig = ConfigManager:CreateConfig("KhoaDevAvadeConfig")
